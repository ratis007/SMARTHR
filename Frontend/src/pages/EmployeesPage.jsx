...
function EmployeeForm({ companies, initialData, onSubmit, onClose }) {
    ...
    <form onSubmit={submit} className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select className="input" value={form.gender ?? 'M'} onChange={set('gender')}>
...
export default function EmployeesPage() {
  ...
  const load = () => {
    ...
    Promise.all([employeesApi.getAll(), companiesApi.getAll()])
      .then(([employeeRes, companyRes]) => {
        setEmployees(employeeRes.data);
        setTotal(employeeRes.meta?.total ?? employeeRes.data.length);
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{total} employe(s) au total</p>
        </div>
        <button onClick={() => setModal(true)} className="btn-primary">
          <PlusIcon className="w-4 h-4" /> Nouvel employe
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state card">
          ...
          <p className="text-gray-500 font-medium">Aucun employe trouve</p>
        </div>
      ) : (
        <div className="table-container overflow-x-auto">
          <table className="w-full text-sm min-w-[980px]">
            <thead><tr>{['Matricule', 'Employe', 'Poste / Departement', 'Entreprise', 'Statut', 'Actions'].map((header) => <th key={header} className="th">{header}</th>)}</tr></thead>
                  ...
                  <tr key={employee.id} className="tr-hover">
                    ...
                    <td className="td">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-gray-900">{employee.lastName} {employee.firstName}</p>
                          {employee.middleName && <p className="text-xs text-gray-400">{employee.middleName}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="td">
                      <p className="font-medium text-gray-800">{employee.position || '-'}</p>
                      {employee.department && <p className="text-xs text-gray-400">{employee.department}</p>}
                    </td>
                    <td className="td text-gray-600">{employee.company?.name || '-'}</td>
                    <td className="td"><span className={status.cls}>{status.label}</span></td>
                    <td className="td">
                      <div className="flex items-center gap-1.5">
                        <Link to={`/employees/${employee.id}`} title="Voir" className="p-1.5 text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 rounded-lg transition-colors"><ChevronRightIcon className="w-4 h-4" /></Link>
      ...
      {modal && (
        <Modal title="Nouvel employe" onClose={() => setModal(false)} size="lg">
          <EmployeeForm companies={companies} onSubmit={handleCreate} onClose={() => setModal(false)} />
        </Modal>
      )}
      {editing && (
        <Modal title="Modifier l'employe" onClose={() => setEditing(null)} size="lg">
          <EmployeeForm companies={companies} initialData={editing} onSubmit={handleUpdate} onClose={() => setEditing(null)} />
        </Modal>
      )}
      {confirm && (
        <ConfirmationModal
          ...
          message={confirm.type === 'delete'
            ? `Archiver ${confirm.item.lastName} ${confirm.item.firstName} ?`
            : `${confirm.item.status === 'active' ? 'Desactiver' : 'Activer'} ${confirm.item.lastName} ${confirm.item.firstName} ?`}
