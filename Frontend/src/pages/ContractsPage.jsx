...
const typeColors = {
  CDI: 'badge-green', CDD: 'badge-blue', STAGE: 'badge-yellow', CONSULTANT: 'badge-purple',
};
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit({ ...form, employeeId: +form.employeeId, salary: +form.salary }); }} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="label">Salaire (CDF) *</label>
          <input type="number" className="input" value={form.salary} onChange={set('salary')} placeholder="0" required />
        </div>
        <div>
      ...
      <div>
        ...
        <textarea className="input" rows={3} value={form.notes} onChange={set('notes')} placeholder="Observations..." />
      </div>
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{contracts.length} contrat(s) · {activeCount} actif(s)</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="empty-state card">
          <DocumentTextIcon className="w-14 h-14 text-gray-200 mb-3" />
        ...
        <div className="table-container">
          <table className="w-full text-sm">
            <thead><tr>
              {['Employé', 'Type', 'Date début', 'Date fin', 'Salaire mensuel', 'Statut'].map((h) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((c) => (
                <tr key={c.id} className="tr-hover">
                  ...
                  <td className="td font-semibold text-gray-900">{Number(c.salary).toLocaleString()} CDF</td>
                  <td className="td">
