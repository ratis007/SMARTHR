...
function LeaveForm({ employees, onSubmit, onClose }) {
  ...
  const days = form.startDate && form.endDate
    ? Math.max(0, Math.ceil((new Date(form.endDate) - new Date(form.startDate)) / 86400000) + 1) : 0;
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit({ ...form, employeeId: +form.employeeId }); }} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><label className="label">Date de fin *</label><input type="date" className="input" value={form.endDate} onChange={set('endDate')} required /></div>
      </div>
      {days > 0 && <div className="bg-indigo-50 rounded-xl px-4 py-2.5 border border-indigo-100 text-sm text-indigo-700 font-semibold">📅 Durée : {days} jour(s)</div>}
      <div><label className="label">Motif</label><textarea className="input" rows={3} value={form.reason} onChange={set('reason')} /></div>
      <div className="flex gap-3 pt-2">
        <button type="submit" className="btn-primary flex-1 justify-center">Soumettre</button>
...
export default function CompanyLeave() {
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            {pendingCount > 0
              ? <span className="text-amber-600 font-bold">{pendingCount} demande(s) en attente</span>
              ...
              {filtered.map(l => {
                ...
                return (
                  <tr key={l.id} className="tr-hover">
                    ...
                    <td className="td text-gray-600 text-xs">{l.startDate} → {l.endDate}</td>
                    <td className="td"><span className="badge-gray">{days}j</span></td>
                    <td className="td"><span className={sc.cls}>{sc.label}</span></td>
                    <td className="td">
                      {l.status === 'pending' && (
                        <div className="flex gap-1.5">
                          <button onClick={() => handleApprove(l.id)} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors"><CheckIcon className="w-4 h-4" /></button>
                          <button onClick={() => handleReject(l.id)}  className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><XMarkIcon className="w-4 h-4" /></button>
                        </div>
                      )}
                    </td>
                  </tr>
