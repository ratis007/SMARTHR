  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit({ ...form, employeeId: +form.employeeId, month: +form.month, year: +form.year, baseSalary: form.baseSalary ? +form.baseSalary : undefined }); }} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><label className="label">Année *</label><input type="number" className="input" value={form.year} onChange={set('year')} required /></div>
      </div>
      ...
      <div className="bg-indigo-50 rounded-xl p-3 border border-indigo-100">
        <p className="text-xs font-bold text-indigo-700 mb-1">🇨🇩 Déductions automatiques RDC</p>
        <p className="text-xs text-indigo-600">CNSS · IPR · INPP · ONEM calculés selon les taux légaux.</p>
      </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="page-header">
        ...
        <button onClick={() => setModal(true)} className="btn-primary"><PlusIcon className="w-4 h-4" /> Générer une fiche</button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="card bg-gradient-to-br from-red-50 to-white border-red-100">
          <p className="text-xs font-bold text-red-400 uppercase tracking-wide">Total déductions</p>
      ...
      {loading ? (
        ...
        <div className="table-container">
          <table className="w-full text-sm">
            <thead><tr>{['Matricule','Employé','Salaire base','Primes','Déductions','Net à payer','Statut','Action'].map(h => <th key={h} className="th">{h}</th>)}</tr></thead>
